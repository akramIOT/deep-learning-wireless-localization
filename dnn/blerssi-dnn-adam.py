...
def create_deep(inp_dim, rate, beta1):
    seed = 7
    np.random.seed(seed)
    model = Sequential()
    ...
    # Compile model
    model.compile(loss=rmse, optimizer=Adam(rate,beta1), metrics=['mse'])
    return model
...
if __name__ == "__main__":
  ...
  model = create_deep(train_x.shape[1],args.learning_rate,args.beta1)
